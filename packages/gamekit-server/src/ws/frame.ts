...
const KNOWN_OPCODES = new Set<number>([0x0, 0x1, 0x2, 0x8, 0x9, 0xa]);
...
export function parseFrames(buffer: Buffer): {
  ...
  while (offset + 2 <= buffer.length) {
    ...
    if (len === 126) {
      if (offset + 4 > buffer.length) break; // need extended length bytes
    ...
    if (payloadStart + len > buffer.length) break; // incomplete payload
