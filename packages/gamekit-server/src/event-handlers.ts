...
import type { ServerHooks, SpriteSnapshot } from './types.js';
...
export class EventHandlers {
   ...
   */
  private async handleCreateRoom(socket: Socket, data: { name?: string } = {}): Promise<string | null> {
   ...
   */
  private async handleJoinRoom(socket: Socket, data: { code?: string; name?: string } = {}): Promise<string | null> {
   ...
   */
  private async handleSpriteSync(
    socket: Socket,
    data: { room?: string; sprites?: SpriteSnapshot[] } = {},
    currentRoomCode: string | null
  ): Promise<void> {
    const code = data.room || currentRoomCode;
    const room = this.roomManager.getRoom(code || '');

    if (!room || !Array.isArray(data.sprites)) return;
   ...
   */
  private handleRequestLeaderboard(socket: Socket, currentRoomCode: string | null): void {
   ...
   */
  private async handleDisconnect(socket: Socket, currentRoomCode: string | null, playerName: string): Promise<void> {
