...
export class Physics {
    ...
    buildTilemapBodies(solidRects, tileSize) {
        for (const rect of solidRects) {
            const body = Matter.Bodies.rectangle(rect.x + rect.width / 2, rect.y + rect.height / 2, rect.width, rect.height, { isStatic: true, friction: 0.8, restitution: 0 });
            Matter.Composite.add(this._world, body);
        }
    }
    // ------------------------------------------------------------------
    ...
    _handleCollision(event, type) {
        for (const pair of event.pairs) {
            ...
            for (const col of this._colliders) {
                if (col.type !== type)
                    continue;
                const match = (col.spriteA === spriteA && col.spriteB === spriteB) ||
                    (col.spriteA === spriteB && col.spriteB === spriteA);
                if (match)
                    col.callback(spriteA, spriteB);
            }
        }
    }
}
