...
export class BinaryWriter {
    ...
    str8(s) {
        const b = TE.encode(s);
        if (b.length > 255)
            throw new RangeError(`string too long for str8: ${s}`);
        this.u8(b.length);
        this.bytes(b);
    }
...
function decodeBinaryServer(data) {
    ...
    if (kind === KIND_SNAP) {
        ...
        for (let i = 0; i < count; i++) {
            ...
            const rot = r.f32();
            const s = r.value();
            const ent = { id, t: tag, x, y, r: rot };
            if (s !== undefined)
                ent.s = s;
            ents[i] = ent;
        }
        const state = r.value();
        const msg = { k: "snap", tick, t, lastSeq, ents };
        if (state !== undefined)
            msg.state = state;
        return msg;
    }
...
function decodeBinaryClient(data) {
    if (typeof data === "string") {
        throw new TypeError("binaryCodec received a text frame");
    }
    const r = new BinaryReader(data);
...
export const jsonCodec = {
    ...
    decodeServer: (data) => JSON.parse(asText(data)),
    decodeClient: (data) => JSON.parse(asText(data)),
};
/** The codec used when none is specified. */
export const defaultCodec = binaryCodec;
