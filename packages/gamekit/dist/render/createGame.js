 ...
 * through untouched.
 */
function withFitDefaults(canvas, config) {
    if (config.fov !== undefined || config.width !== undefined)
        return config;
    const cssWidth = canvas.getBoundingClientRect().width || canvas.clientWidth || canvas.width || 480;
    return {
        ...config,
